...
import createTokenQueryColumns from './token-query/TokenQueryColumns';
import TokenQueryResultPanel from './token-query/TokenQueryResultPanel';
import TokenQuerySearchPanel from './token-query/TokenQuerySearchPanel';
import TokenQuerySummarySection from './token-query/TokenQuerySummarySection';
import {
    formatAccessDate,
    formatBalanceValue,
    formatDate,
    ...
    formatUsageValue,
    renderTimestamp,
} from './token-query/TokenQueryFormatters';
...
const LogsTable = () => {
    ...
    const fetchData = async (params = {}) => {
        ...
        const prev = tabData[activeTabKey] || { balance: 0, usage: 0, accessdate: "未知", tokenValid: false, hasFetchedBalance: false, page: 1, total: 0 };
        ...
        try {
            if (process.env.REACT_APP_SHOW_DETAIL === "true") {
                ...
                if (payload && payload.success === true) {
                    ...
                    setActiveKeys(process.env.REACT_APP_SHOW_DETAIL === "true" ? ['2'] : []);
                } else {
    ...
    const columns = createTokenQueryColumns({ copyText });

    const copyTokenInfo = (e) => {
        e.stopPropagation();
        const activeTabData = tabData[activeTabKey] || {};
        const { balance, usage, accessdate } = activeTabData;
        const info = `令牌总额: ${formatBalanceValue(balance)}
剩余额度: ${formatRemainingValue(balance, usage)}
已用额度: ${formatUsageValue(balance, usage)}
有效期至: ${formatAccessDate(accessdate)}`;
        copyText(info);
    };
    ...
    const tokenInfoEnabled = process.env.REACT_APP_SHOW_BALANCE === "true";
    const detailEnabled = process.env.REACT_APP_SHOW_DETAIL === "true";

    const renderContent = () => (
        <div className="token-query-page">
            <TokenQuerySearchPanel
            ...
            <TokenQuerySummarySection
                enabled={tokenInfoEnabled}
                activeTabData={activeTabData}
                copyTokenInfo={copyTokenInfo}
            />

            <TokenQueryResultPanel
                ...
                fetchData={fetchData}
                ignoreNextPageChangeRef={ignoreNextPageChangeRef}
            />
        </div>
    );
